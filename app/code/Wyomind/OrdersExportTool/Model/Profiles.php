...
class Profiles extends \Magento\Framework\Model\AbstractModel
{
    ...
    private function getItemCollection()
    {
        ...
        if (is_array($this->_params['store_id'])) {
            ...
            $values = explode(",", (string) $this->_params['store_id']);
        }
        // specific order
        if (isset($this->_params['increment_id']) && $this->_params['increment_id'] != '') {
            $searchCriteria->addFilter('increment_id', (int) $this->_params['increment_id'], 'eq');
        }
        //scope
        ...
        //order
        $searchCriteria->addFilter('status', explode(',', (string) $this->_params['states']), 'in');
        //scope
        $customerGroups = explode(",", (string) $this->_params['customer_groups']);
        $searchCriteria->addFilter('customer_group_id', explode(',', (string) $this->_params['customer_groups']), 'in');
            ...
            foreach ($this->_attributesFilter['order'] as $attributeFilter) {
                if ($attributeFilter->checked) {
                    if ($attributeFilter->condition == 'in' || $attributeFilter->condition == 'nin') {
                        $value = explode(',', (string) $attributeFilter->value);
                    } else {
     ...
     */
    public function generate($request = null, $preview = false, $incrementId = null)
    {
        try {
...
";
            ...
            if (isset($incrementId)) {
                $this->_params['increment_id'] = $incrementId;
            }
            ...
            foreach ($collection as $order) {
                ...
                if ($this->_params['flag'] && $this->_params['single_export'] && in_array($this->_params['id'], explode(',', (string) $order->getExportedTo())) && !$this->_isPreview) {
                    continue;
                }
                $ioSingle = null;
                    ...
                    foreach ($values as $value) {
                        ...
                        if (is_array($value)) {
                            try {
                                ...
                                // return the json encoded value if it can't be imploded
                ...
                if (!empty($orderPattern)) {
                    ...
                    if (!$this->_isPreview && $this->_params['repeat_for_each']) {
                        ...
                        if ($this->helper->isXml($this->_params) && strlen(trim((string) $this->_params['footer'])) > 1) {
                            $ioSingle->write($this->_params['footer']);
                        } elseif (strlen(trim((string) $extraFooter)) > 1) {
                ...
                if ($this->_params['update_status'] && !$this->_isPreview && $orderPattern != '') {
                    $status = explode('-', (string) $this->_params['update_status_to']);
                ...
                if ($this->_params['flag'] && !$this->_isPreview && $orderPattern != '') {
                    $flags = explode(',', (string) $order->getExportedTo());
            ...
            if (!$this->_isPreview && !$this->_params['repeat_for_each']) {
                ...
                if ($this->_params['type'] == 1) {
                    if (strlen(trim((string) $this->_params['footer'])) > 1) {
                        $ioFull->write($this->_params['footer']);
                    }
                } else {
                    if (strlen(trim((string) $extraFooter)) > 1) {
                ...
                if ($this->_params['mail_enabled'] && count($filesList) > 0 && $increment > 1) {
                    if ($this->_params['mail_one_report']) {
                        if ($this->_params['mail_zip']) {
                            ...
                            $zip->close();
                            $mails = explode(',', (string) $this->_params['mail_recipients']);
