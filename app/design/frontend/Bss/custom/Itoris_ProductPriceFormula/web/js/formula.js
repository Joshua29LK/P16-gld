...
Itoris.PriceFormula = {
    ...
    initialize: function(conditions, optionsData, specialPrice, dataBySku, productId, conversionRate, taxInfo, tierPrices) {
        ...
        this.getPriceBox().on('reloadPrice', function(ev){
            ...
            try {
                ...
                var normal_price = jQuery('[data-role="priceBox"][data-product-id="'+curObj.productId+'"] [data-price-type="oldPrice"] .price')?.[0].innerHTML;
                normal_price = normal_price?.replace(/[^0-9]+/g,"") / (normal_price?.indexOf(decimalSymbol) > -1 ? 100 : 1);

                var spe_price = jQuery('#itoris_dynamicproductoptions_popup_price [data-price-type="finalPrice"] .price');
                if (!spe_price[0]) spe_price = jQuery('[data-role="priceBox"][data-product-id="'+curObj.productId+'"] [data-price-type="finalPrice"] .price')?.[0].innerHTML;
                spe_price = spe_price?.replace(/[^0-9]+/g,"") / (spe_price?.indexOf(decimalSymbol) > -1 ? 100 : 1);
    ...
    updatePriceInPriceBoxObject: function(basePriceObj, finalPriceObj){
        ...
        var bssPrice = parseFloat(curObj.finalPrice).toFixed(2);
