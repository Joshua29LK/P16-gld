define([
    ...
    return Component.extend({
        ...
        selectDefaultProduct: function() {
            ...
            var step = this.getUrlParameter('step_id');
            var sku = this.getUrlParameter('sku');
            if (this.selector == '#configurator_step_' + step && sku != '') {
                firstItem = $('#configurator_step_' + step).find('.sku-' + sku);
            }
        ...
        initCarousel: function () {
            var step = this.getUrlParameter('step_id'),
                ...
                startPosition = 0;
            if (products !== undefined && this.selector == ('#configurator_step_' + step)) {
                products.forEach(function (field, index) {
                    if (field.product_id && field.product_id == sku) {
                        startPosition = index;
                        return;
                    }
                })
