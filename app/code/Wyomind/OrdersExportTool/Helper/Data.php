...
&nbsp;&nbsp;- ID: " . $item->getId());
    ...
    {
        $matches = [];
        preg_match_all("/(?<script><\\?php(?<php>.*)\\?>)/sU", (string) $output, $matches);
        $i = 0;
        foreach (array_values($matches["script"]) as $phpCode) {
            $parsed = preg_replace("#({{[^}]*}})#m", "\"\$1\"", (string) $phpCode);
            $output = str_replace((string) $phpCode, (string) $parsed, (string) $output);
        }
        return $output;
    }
    /**
    ...
    public function executePhpScripts($preview, $output, $order = null, $data = [], $item = null, $type = 1)
    {
        ...
        foreach (array_values($matches["php"]) as $phpCode) {
            ...
            $output = str_replace((string) $matches["script"][$i], (string) $val, $output);
            $i++;
        }
        return $output;
    }
