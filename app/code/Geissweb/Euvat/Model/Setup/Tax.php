...
class Tax
{
    ...
    const TC_PRODUCTS_SHIPPING_STANDARD = 3;
    ...
    public function runSetup(array $post)
    {
        try {
            ...
            if (isset($post['create_tax_rules']) && $post['create_tax_rules'] == 1) {
                ...
                $this->addTaxClasses($this->setupHelper->getTaxClassesInsertArray((bool)$post['use_digital_rule']));
    ...
    private function addTaxRates(array $rates)
    {
        ...
        foreach ($rates as $rate) {
            ...
            } elseif (stristr(__($rate['code'])->getText(), __(' VAT zero/exempt')->getText())) {
     ...
     * @return bool
     */
    private function addTaxRules($useMSRate = true, $withReduced = true, $withDigital = false)
    {
        try {
     ...
     * @return void
     */
    private function applyStoreConfig(string $baseCc, string $key, string $vatId, string $installType, bool $withRules)
    {
        try {
