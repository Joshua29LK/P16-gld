...
abstract class Base extends Action
{
    ...
    protected function prepareSenderData($request)
    {
        ...
        } elseif (!$this->senderName || !$this->senderEmail) {
            $result = [
                'error' => true,
                'message' => __('Your Name and Email are required. Please retry.')
            ];
        }
        return $result;
    }
    ...
    protected function validateEmailAddress($emailAddresses, $type)
    {
        foreach ($emailAddresses as $emailAddress) {
            if (!empty($emailAddress) && !\Zend_Validate::is($emailAddress, EmailAddress::class)) {
                throw new \Exception(__('Invalid %1', $type));
            }
        }
    }
}
